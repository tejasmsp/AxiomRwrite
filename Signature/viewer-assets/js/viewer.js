...
(function ($, undefined) {
    ...
    function Viewer(element, options) {
        ...
        this.viewerNodes = {
            ...
            //$save: viewer.$dom.find("[data-pcc-save]"),
            ...
            $SignatureButton: viewer.$dom.find("[data-pcc-ApplySign]")
        };
        ...
        function SearchText() {
            $.ajax({
                ...
                url: "/viewer-webtier/pcc.ashx/v2/viewingSessions/" + $("#hdn").text() + "/searchTasks",
                data: "{\n\t\"input\": {\n\t\t\"searchTerms\": [\n\t\t\t{\n\t\t\t\t\"type\": \"simple\",\n\t\t\t\t\"pattern\": \"patient signature\"\n\t\t\t}\n\t\t]\n\t}\n}",
                success: function (data) {
                    ...
                    $.ajax({
                        ...
                        url: "/viewer-webtier/pcc.ashx/v2/searchTasks/" + processID + "/results",
                        success: function (data) {
                            var sigVal;
                            var sigs = JSON.parse((localStorage.getItem("pccvSignatures")));
                            for (let i = 0; i < sigs.values.length; i++) {
                                if (sigs.values[i].lastSelected == true) {
                                    sigVal = sigs.values[i];
                                    break;
                                };
                            };
                            console.log(viewer.currentMarks);
                            var adjustX = 80;
                            var adjustY = 10;

                            //var queue = new Queue();
                            //PCCViewer.Signatures.forEach(function (signature) {
                            ...
                            if (data.results.length > 0) {
                                ...
                                for (k = 0; k < data.results.length; k++) {
                                    ...
                                    if (sigVal.type == "text") {
                                        ...
                                        var mark = viewer.viewerControl.addMark(data.results[k].pageIndex+1, "TextSignature");
                                        var documentX = data.results[k].lineRectangles[0].x + adjustX;
                                        var documentY = data.results[k].lineRectangles[0].y - adjustY;
                                        mark.setRectangle({ x: documentX, y: documentY, width: 100, height: 30 });
                                        mark.setText(sigVal.text);
                                        mark.setFontName(sigVal.fontName);

                                    } else if (sigVal.type == "path") {

                                        var mark = viewer.viewerControl.addMark(data.results[k].pageIndex+1, "FreehandSignature");
                                        var documentX = data.results[k].lineRectangles[0].x + adjustX;
                                        var documentY = data.results[k].lineRectangles[0].y - adjustY;
                                        mark.setRectangle({ x: documentX, y: documentY, width: 100, height: 30 });
                                        mark.setPath(sigVal.path);
        ...
        this.bindMarkup = function () {
            ...
            viewer.viewerNodes.$SignatureButton.on("click", function () {
        ...
        this.search = (function () {
            ...
            var nextResultClickHandler = function (nextResultBtn) {
                ...
                if (activeSearchResultId === undefined) {
                    ...
                    $activeSearchResult = results.find("[data-pcc-search-result-id='" + activeSearchResultId + "']").next();
            ...
            var previousResultClickHandler = function (previousResultBtn) {
                ...
                if (activeSearchResultId === undefined) {
                    ...
                    $activeSearchResult = results.find("[data-pcc-search-result-id='" + activeSearchResultId + "']").prev();
        ...
        this.eSignature = (function () {
            ...
            function insertSignatureView(signature, domElem, clickHandler, includeButtons) {
                ...
                Savebutton.className = 'pcc-icon pcc-icon-save';
        ...
        var fileDownloadManager = (function () {
            ...
            function onSuccessDownloadURL(url, $overlay, $overlayFade) {
                showOverlay($overlay, $overlayFade, { mode: 'complete' })
                    .on('click', '.pcc-overlay-download', function () {
                        //debugger;
                        //var urltemp = "http://localhost:62514/"+url;
                        //$.ajax({
                        //    type: "GET",
            ...
            //       // url: "http://localhost:51617/Secured/Data/GetMRBillingData.aspx/AddFileToPartNew",
            //        data: JSON.stringify({
            ...
            //            if (result.success) {
            //                //Axiom.misc.empDocumentView.refresh();
